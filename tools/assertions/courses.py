from clients.courses.courses_model import UpdateCourseRequestSchema, UpdateCourseResponseSchema, CourseSchema, \
...
def assert_update_course_response(
    ...
    if request.title is not None:
        assert_equals(response.course.title, request.title, "title")

    if request.max_score is not None:
        assert_equals(response.course.max_score, request.max_score, "max_score")

    if request.min_score is not None:
        assert_equals(response.course.min_score, request.min_score, "min_score")

    if request.description is not None:
        assert_equals(response.course.description, request.description, "description")

    if request.estimated_time is not None:
        assert_equals(response.course.estimated_time, request.estimated_time, "estimated_time")
...
def assert_create_course_response(request: CreateCourseRequestSchema, response: CreateCourseResponseSchema):
    ...
    :param response: CreateCourseResponseSchema
    """
    assert_equals(response.course.title, request.title, "title")
    assert_equals(response.course.max_score, request.max_score, "max_score")
    assert_equals(response.course.min_score, request.min_score, "min_score")
    ...
    assert_equals(response.course.preview_file.id, request.preview_file_id, 'preview_file')
    assert_equals(response.course.created_by_user.id, request.created_by_user_id, 'created_by_user')
